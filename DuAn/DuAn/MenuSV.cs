...
namespace DuAn
{
    public partial class MenuSV : Form
    {
        ...
        private void btntimkiem_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(timkiem))
            {
                var result = from sv in db.SinhViens
                             join mh in db.MonHocs on sv.MaMh equals mh.MaMh
                             where sv.MaSv.StartsWith(timkiem) || sv.HoTen.StartsWith(timkiem)
                             select
                             new
                             {
                                 ID = sv.MaSv,
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string filepath = "D:\\FPT polytechnic\\C#3\\learn\\DuAn\\DuAn\\hotro.txt";
            if (text == "")
            {
                check = false;
        ...
        private void Btn_hienthi(object sender, EventArgs e)
        {
            LoadDataSV();
            txttimkiem.Text = "";
        }
